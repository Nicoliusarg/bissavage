// scripts/update-bis.js — V1 fallback sólido (incluye gear)
// Requiere: WCL_V1_KEY, BLIZZARD_* (y mantiene WCL v2 por si luego querés volver).
// Limita páginas y mete una pequeña pausa para evitar 429.
...
const MAX_PAGES = 1;      // páginas por encuentro (subí a 2 si querés más)
const PAUSE_MS  = 150;    // pausa entre requests para evitar 429
...
// /v1/zones => lista zones + encounters
...
// /v1/rankings/encounter/{encounterID}?class=..&spec=..&difficulty=..&metric=..&includeCombatantInfo=true&page=..
// Devuelve un array de rankings (cada uno con 'gear')
async function fetchEncounterRankings(encId, className, specName, metric, difficulty, page){
  return v1(`rankings/encounter/${encId}`, {
    class: className, spec: specName, difficulty, metric,
    includeCombatantInfo: true, page
  });
}
  ...
  for (const diff of [5,4]) {        // Mythic → Heroic
    ...
    for (const enc of encounters){
      for (let page=1; page<=MAX_PAGES; page++){
        try{
          const ranks = await fetchEncounterRankings(enc.id, className, specName, metric, diff, page);
          ...
          // 404/400 cuando no hay datos para esa combinación → seguí
          break;
        }
      }
    }
    ...
    if (out.length) return out; // listo
  }
  return [];
}
